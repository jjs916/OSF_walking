...
//cout << "Lambda\n"<< rd_.lambda.block(0,0,6,6) << endl << endl;
    ...
    VectorQd TaskControlTorqueMotor(RobotData &rd_, VectorXd f_star, Eigen::MatrixVVd B, Eigen::MatrixVVd B_inv, Eigen::VectorXd &force)
    {
        int task_dof = rd_.J_task.rows();
        //rd_.J_task = J_task;
        rd_.J_task_T = rd_.J_task.transpose();
        ...
        // return rd_.W_inv * (rd_.Q_T_ * (rd_.Q_temp_inv * (lambda_m * f_star)));
    }
    ...
    VectorQd ContactForceRedistributionTorqueJS(RobotData &Robot, VectorQd command_torque, double ratio, int supportFoot)//
    {
        ...
        if (contact_dof_ == 12)
        {
            ...
            ForceRedistributionTwoContactMod2(0.8, foot_length, foot_width, 1.0, 0.9, 0.9, Rotyaw * P1_, Rotyaw * P2_, F12, ResultantForce_, ResultRedistribution_, eta);
            Robot.fc_redist_ = force_rot_yaw.transpose() * ResultRedistribution_;
            ...
            if (supportFoot == 0)
            {
                right_master = 1;//오른발 지지
            }
            else
            {
                right_master = 0.0;//왼발 지지
            }
